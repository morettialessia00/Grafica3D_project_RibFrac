...
volrend_result volrend_dvr(float3 vertexLocal, float2 uv)
{
    ...
    // Depth cueing: luminosità dei campioni sul retro del volume (0 = nero, 1 = nessun effetto).
    // 0.12 = lo sfondo si smorza al 12%. Abbassa per un effetto più marcato, alza per più leggero.
    #define DEPTH_CUE_FAR_BRIGHTNESS 0.12f
    ...
    for (int iStep = 0; iStep < raymarchInfo.numSteps; iStep++)
    {
        ...
        // Depth cueing: campioni vicini alla camera pieni, lontani smorzati.
        // t = 0 sul fronte (vicino alla camera), t = 1 sul retro (lontano).
        // t si ricalcola ogni frame rispetto alla camera → l'effetto segue le rotazioni.
        src.rgb *= lerp(1.0f, DEPTH_CUE_FAR_BRIGHTNESS, t);
